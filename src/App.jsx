...
<div className="overflow-x-auto">
  <div className="relative min-w-fit h-full pointer-events-none">
    {/* Base Grid: always visible */}
    <div className="grid grid-cols-7 grid-rows-24 border-t border-l border-[#6246EA] pointer-events-none">
      {Array.from({ length: 24 * 7 }).map((_, i) => (
        <div
          key={i}
          className="border border-[#6246EA] w-[150px] h-[60px] pointer-events-none"
        ></div>
      ))}
    </div>

    {/* Scheduled Items: layered on top if any */}
    <div className="absolute top-0 left-0 w-full h-full pointer-events-none">
      {sched.map((s, i) => (
        <div
          key={i}
          className="absolute border-2 border-[#6246EA] bg-[#D1D1E9]/80 rounded-lg px-4 py-2 text-center pointer-events-auto"
          style={{
            gridColumnStart: s.column,
            gridRowStart: s.rowStart + 1,
            gridRowEnd: s.rowEnd + 1,
            top: `${s.rowStart * 60}px`, // 60px height per row
            left: `${(s.column - 1) * 150}px`, // 150px width per column
            height: `${(s.rowEnd - s.rowStart) * 60}px`,
            width: `150px`,
          }}
        >
          <h3 className="text-base font-semibold text-[#2B2C34] custom-font">{s.title}</h3>
